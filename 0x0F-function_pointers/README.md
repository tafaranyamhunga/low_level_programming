...
- [ ] The first bytes code of the function
...
to the variable `f` of the type pointer to a function that does not take any argument and does not return anything, you would do (check all correct answers if there is more than one):
...
- [ ] A pointer to a function that takes an array of `int` and `float` as parameters and returns nothing

- [ ] An array of pointers to functions that take an`int` and `float` as parameters and returns nothing
